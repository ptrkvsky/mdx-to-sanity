...
import { createMockOpenAIFetch, createMockOpenAIFetchError, createMockOpenAIResponse, } from "../../__tests__/mocks/openai.mock.js";
describe("createOpenAIMarkdownTransformer", () => {
    ...
    it("should transform article to markdown with SEO metadata", async () => {
        ...
        const openAIResponse = createMockOpenAIResponse(transformedContent, metadata);
...
===END===`;
        ...
        expect(result).toContain(mockArticle.title);
        expect(result).toContain(mockArticle.content);
        expect(consoleErrorSpy).toHaveBeenCalledWith("Error parsing combined response:", expect.any(Error));
