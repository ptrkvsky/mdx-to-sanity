...
function calculateMetadata(content) {
...
function createCombinedPrompt(article) {
...
function parseCombinedResponse(response) {
    try {
        const contentMatch = response.match(/===CONTENT===\s*([\s\S]*?)\s*===METADATA===/);
        const metadataMatch = response.match(/===METADATA===\s*([\s\S]*?)\s*===END===/);
...
async function callOpenAI(apiKey, prompt, model = "gpt-4o-mini", maxTokens = 4000) {
...
export function createOpenAIMarkdownTransformer(apiKey) {
    return {
        transformToMarkdownWithSEO: async (article) => {
            ...
            try {
                ...
                console.error("OpenAI transformation failed, using original content:", error);
