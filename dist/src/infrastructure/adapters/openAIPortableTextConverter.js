...
function createPortableTextPrompt(markdown) {
...
function parseBlockContentResponse(response) {
...
export function createOpenAIPortableTextConverter(apiKey) {
    return {
        convertMarkdownToPortableText: async (markdown) => {
            try {
                ...
                throw new Error(`OpenAI conversion failed: ${error instanceof Error ? error.message : String(error)}`);
            }
        },
    };
}
