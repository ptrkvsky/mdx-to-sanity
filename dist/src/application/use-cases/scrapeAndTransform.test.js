...
describe("scrapeAndTransform", () => {
    it("should scrape and transform article to markdown", async () => {
        ...
        // Act
        const scrapeAndTransformArticle = scrapeAndTransform(mockScraper, mockTransformer);
        ...
        expect(mockTransformer.transformToMarkdownWithSEO).toHaveBeenCalledWith(mockArticle);
        expect(mockTransformer.transformToMarkdownWithSEO).toHaveBeenCalledTimes(1);
    ...
    it("should propagate error when scraper fails", async () => {
        ...
        // Act & Assert
        const scrapeAndTransformArticle = scrapeAndTransform(mockScraper, mockTransformer);
        await expect(scrapeAndTransformArticle("https://example.com")).rejects.toThrow("Scraping failed");
    ...
    it("should propagate error when transformer fails", async () => {
        ...
        // Act & Assert
        const scrapeAndTransformArticle = scrapeAndTransform(mockScraper, mockTransformer);
        await expect(scrapeAndTransformArticle("https://example.com")).rejects.toThrow("Transformation failed");
        expect(mockScraper.scrape).toHaveBeenCalledWith("https://example.com");
        expect(mockTransformer.transformToMarkdownWithSEO).toHaveBeenCalledWith(mockArticle);
    });
    it("should save markdown file when repository is provided", async () => {
        ...
        // Act
        const scrapeAndTransformArticle = scrapeAndTransform(mockScraper, mockTransformer, mockRepository);
        const result = await scrapeAndTransformArticle("https://example.com");
        // Assert
        expect(result).toBe(mockMarkdown);
        expect(mockRepository.saveMarkdown).toHaveBeenCalledTimes(1);
        expect(mockRepository.saveMarkdown).toHaveBeenCalledWith(expect.stringMatching(/^2024-01-15-test-article\.md$/), mockMarkdown);
    });
    ...
    it("should not fail when repository save fails", async () => {
        ...
        // Assert
        expect(result).toBe(mockMarkdown);
        expect(consoleErrorSpy).toHaveBeenCalledWith("Failed to save markdown file:", expect.any(Error));
...
# Test Content`;
    ...
    it("should handle markdown with missing date in frontmatter", async () => {
        ...
        // Assert
        expect(result).toBe(mockMarkdown);
        expect(mockRepository.saveMarkdown).toHaveBeenCalledWith(expect.stringMatching(/^\d{4}-\d{2}-\d{2}-test-article\.md$/), mockMarkdown);
    });
