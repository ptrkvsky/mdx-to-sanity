...
const createMetadataPrompt = (article: Article): string => {
...
const createContentTransformationPrompt = (article: Article): string => {
...
const parseOpenAIResponse = (
	...
	try {
		...
		const parsed: OpenAIResponse = JSON.parse(cleanedResponse);
...
export function createOpenAIEnricher(apiKey: string): ArticleEnrichiService {
	return {
		enrichArticle: async (article: Article): Promise<ArticleEnrichi> => {
			...
			let openAIMetadata: Partial<ArticleEnrichi["metadata"]> = {};

			// Appel 1 : Générer les métadonnées SEO
			try {
				const metadataPrompt = createMetadataPrompt(article);
				const metadataResponse = await callOpenAI(
					apiKey,
					metadataPrompt,
					"gpt-3.5-turbo",
					500,
			...
			try {
				...
				const transformedContent = await callOpenAI(
					apiKey,
					contentPrompt,
					"gpt-4o-mini",
					4000,
			...
			const articleWithStructuredContent: Article = {
				...article,
